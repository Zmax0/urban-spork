...
class CipherTest {
    ...
    private byte[] cipherTest(ShadowsocksCipher cipher, ShadowsocksKey key, byte[] in) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            ByteBuf buf = Unpooled.buffer().writeBytes(divided[0]);
            buff.writeBytes(cipher.encrypt(buf, key));
            temp = divided[1];
        }
        buff.writeBytes(cipher.encrypt(Unpooled.buffer().writeBytes(temp), key));
        byte[] encrypt = new byte[buff.readableBytes()];
        buff.readBytes(encrypt);
        temp = encrypt;
        for (int i = 0; i < 100; i++) {
            byte[][] divided = randomDivide(temp);
            buff.writeBytes(cipher.decrypt(Unpooled.buffer().writeBytes(divided[0]), key));
            temp = divided[1];
        }
        buff.writeBytes(cipher.decrypt(Unpooled.buffer().writeBytes(temp), key));
